...
export const Slide = (): JSX.Element => {
  ...
  const authenticityCards = [
    ...
    },
    {
      ...
      description: (
        <>
          <span>The application synchronizes with our cart, allowing shoppers to </span>
          <span className="font-bold">see the amount of their purchases</span>
          <span> in real time, receive personalized offers and </span>
          <span className="font-bold">recommendations based on preferences.</span>
        </>
      ),
    },
  ];
    ...
    <div className="flex flex-col items-center relative bg-white overflow-hidden">
      ...
      <header className="flex w-full items-center justify-between px-4 sm:px-8 md:px-[139px] py-[13px] relative bg-white shadow-[0px_1px_0px_#cbcbcb] z-10">
        ...
        <nav className="hidden md:flex items-center gap-[50px]">
          <a href="#who-are-we" className="font-normal text-black text-[24px] sm:text-[32px]">
            who are we
          </a>
          <a href="#contact-us" className="font-normal text-black text-[24px] sm:text-[32px]">
            contact us
          </a>
        </nav>
      </header>
        ...
        <div className="container mx-auto flex flex-col lg:flex-row justify-between items-center">
          <div className="flex flex-col gap-8 sm:gap-[78px] max-w-full lg:max-w-[723px] px-4 sm:px-0">
            <div className="flex flex-col gap-4 sm:gap-[37px]">
              <h1 className="font-normal text-black text-5xl sm:text-[96px] leading-tight sm:leading-[96px]">
                Make your shopping easier
              </h1>
              <p className="font-normal text-black text-xl sm:text-[32px]">
            ...
            <div className="flex flex-col items-center">
              <a
                href="https://drive.google.com/drive/folders/1ZbQhmUfDjAtYcDsqX-EQg0C5n0UeYCUY"
                target="_blank"
                rel="noopener noreferrer"
              >
                <Button className="w-full sm:w-auto h-[60px] sm:h-[81px] px-4 sm:px-[21px] py-2 sm:py-[27px] bg-[#ec2649] rounded-xl sm:rounded-2xl text-base sm:text-xl text-white flex items-center justify-center">
                  <img
              ...
              <div className="flex justify-center gap-[40px] mt-[10px]">
                <div className="flex items-center gap-[5px]">
                  ...
                  />
                  <span className="text-[#474a52] text-sm">54</span>
                </div>
                <div className="flex items-center gap-[5px]">
                  <img
                    className="w-4 h-4"
                  ...
                  />
                  <span className="text-[#474a52] text-sm">272</span>
                </div>
              </div>
            </div>
          </div>
        ...
        {features.map((feature, index) => (
          ...
          >
            <img
              ...
              src={feature.icon}
            />
            <h2 className={`font-bold ${feature.textColor} text-3xl sm:text-[40px] mb-4`}>
              {feature.title}
            </h2>
            <p className={`font-normal ${feature.textColor} text-xl sm:text-[32px] leading-7 sm:leading-[38px]`}>
              {feature.description}
            </p>
          </div>
        ))}
      ...
      >
        <div className="container mx-auto flex flex-col lg:flex-row justify-between items-center px-4 sm:px-0">
          ...
          <div className="flex flex-col gap-6 sm:gap-[39px] max-w-full lg:max-w-[855px]">
            <h2 className="font-bold text-white text-5xl sm:text-8xl text-center lg:text-left">
              WHO ARE WE
            </h2>
            <p className="font-bold text-white text-xl sm:text-[32px]">
              ...
              app that streamlines the supermarket shopping experience...
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="relative w-full py-12 sm:py-[71px] bg-white overflow-hidden">
        <div className="container mx-auto relative px-4 sm:px-0">
          ...
          <h2 className="font-bold text-black text-5xl sm:text-8xl mb-12 lg:mb-0 lg:absolute lg:w-[561px] lg:left-[-216px] lg:top-[390px] lg:-rotate-90">
            AUTHENTICITY
          </h2>
          ...
          <div className="flex flex-col gap-8 sm:gap-[57px] lg:ml-[300px]">
            {authenticityCards.map((card, index) => (
              <Card
                key={index}
                className="w-full lg:w-[1040px] rounded-[30px] sm:rounded-[50px] shadow-[10px_8px_16px_4px_#00000057]"
              >
                <CardContent className="p-6 sm:p-12 flex flex-col sm:flex-row items-start gap-8 sm:gap-16">
                  <div className="w-[60px] h-[60px] sm:w-[94px] sm:h-[94px] bg-[#e7e7e7] rounded-full flex items-center justify-center">
                    <div className="text-black text-3xl sm:text-5xl text-center">{card.id}</div>
                  </div>
                  <div>
                    <h3 className="font-bold text-black text-3xl sm:text-5xl mb-4">{card.title}</h3>
                    <div className="font-normal text-black text-xl sm:text-[32px]">
                      {card.description}
                    </div>
                  </div>
                </CardContent>
      ...
      >
        <h2 className="font-bold text-[#ec2649] text-5xl sm:text-8xl text-center mb-8 sm:mb-16">
          contact us
        </h2>
        <img
